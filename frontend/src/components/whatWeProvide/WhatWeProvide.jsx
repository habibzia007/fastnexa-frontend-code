...
const WhatWeProvide = () => {
    ...
    const fetchServicesHeaderData = () => {
        http.get('/header-api')
            .then((res) => {
                setServicesHeader(res.data); // Set API response data to state
            })
            .catch((err) => {
                console.error('Error fetching services data:', err);
            });
    ...
    console.log("this is service", serviceHeading)
    ...
    return (
        <div className="relative pt-0 lg:pt-0" style={backgroundImageStyle}>
            <div className="container mx-auto px-4 sm:px-6 pt-10 w-full max-w-[1184px]">
                ...
                <h3
                    className="font-Poppins font-semibold text-2xl sm:text-3xl lg:text-4xl text-[#232F3B] text-center mt-2"
                    dangerouslySetInnerHTML={{
                        __html: serviceHeading.homepage_serv_api?.section_heading || "",
                    }}
                ></h3>
                <div
                    className="mt-8 grid grid-cols-1 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-4 lg:gap-8">
