...
const AboutUsServicesCustomise = ({ serviceFeatures, baseURL }) => {
    console.log("this is", serviceFeatures);
    return (
        <div className="max-w-[1184px] mx-auto px-4 sm:px-6 lg:px-8">
            <div className="space-y-[50px] lg:space-y-[62px]">
                {/* eslint-disable-next-line react/prop-types */}
                {serviceFeatures.map((post) => (
                    <div
                        key={post.id}
                        className={`flex flex-col lg:flex-row ${
                            post.id % 2 === 1 ? "lg:flex-row-reverse" : ""
...
export default AboutUsServicesCustomise;
